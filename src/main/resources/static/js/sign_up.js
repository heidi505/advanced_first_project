...
async function doubleCheck (username){
	try {
		let promiseData = await fetch(`/check/username?username=${username}`);
		let result = await promiseData.json();
		alert(result.data);
...
async function bringNumber(email){
	try {
		let promiseData = await fetch(`/check/email?email=${email}`);
	}catch (error){
		alert(error);
	}
}
...
async function verifyNumber (key){
	try {
		let promiseData = await fetch(`/verify/email?key=${key}`);
		let result = await promiseData.json();
		alert(result.data);
		return result.success;
	}catch (error){
...
window.onload = function(){
	...
	let verifyInput = document.getElementById('verify_number');
