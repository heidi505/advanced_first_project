...
function insertCElement(dataList){
	const canceledTrip =  document.getElementById('canceled_trip');
    canceledTrip.innerHTML = '';
	let myTripCountLabel = '';
	let text = '';
	let isPayed = '';
	let aTag = '';
	...
	for(let i=0; i<3; i++){
    	...
    	aTag.appendChild(span);
    	myTripCountLabel.appendChild(aTag);
    	canceledTrip.appendChild(myTripCountLabel);
	}
    //결제 여부 버튼 끝
    // 여기부터 반복문 
    dataList.forEach((data)=>{
		...
		if(lastYear == '0000'){
			lastYear = year;
    		yearDiv = makeYearTag(year);
    		canceledTrip.appendChild(yearDiv);
		} 
		if(year != lastYear){
			yearDiv = makeYearTag(year);
    		canceledTrip.appendChild(yearDiv);
    	...
    	let beforePaymentLabel = '';
    	if(data.isPayed){
    		beforePaymentLabel = makeElement('span','cancle_payment_label');
    		text = document.createTextNode('결제취소');	
		} else {
			beforePaymentLabel = makeElement('span','cancle_payment_label');
    		text = document.createTextNode('예약취소');	
		}
    	beforePaymentLabel.appendChild(text);
    	li.appendChild(beforePaymentLabel);
    	myTripContentTop.appendChild(li);
    	...
    	myTripContentTop.appendChild(li);
    	// my_trip_content_top 끝(검사 끝)
    	// my_trip_content_mid 시작(아마 함수화 가능? 파라미터 data로)
    	...
    	myTripBox.appendChild(myTripItem);
    	myTripList.appendChild(myTripBox);
    	canceledTrip.appendChild(myTripList);
    });	
    //반복문 끝
}
...
// 어떻게든 함수화 하고 싶지만 .... 어렵다.
function makeMyTripContentMid(data){
	    let myTripContentMid = makeElement('ul','my_trip_content_mid');
    	let myTripContentTit = makeElement('li','my_trip_content_tit');
    	let airlineIconImg = makeElement('div','airline_icon_img');
    	let koreanName = '';
    	...
    	let p = makeElement('p');
    	// 이것도 영문 데이터를 한글 데이터로 바꿔 text안에 넣어줘야 함
    	let text = document.createTextNode(` [${koreanName}] 부산 - 도쿄/나리타`);
    	p.appendChild(text);
    	div.appendChild(p);
    	p = makeElement('p');
    	text = document.createTextNode('예약번호 3018-4609');
